...
from kerykeion import AstrologicalSubject, Kerykeionimport os
import datetime
import pytz
...
# --- NEW, STABLE ASTROLOGY CALCULATION LOGIC ---
...
def calculate_astro_details(date_str, time_str, lat, lon):
    """
    Calculates D1 and D9 charts using the reliable Swiss Ephemeris library.
    """
    # 1. Convert local time to UTC Julian Day
    year, month, day = map(int, date_str.split('-'))
    hour, minute = map(int, time_str.split(':'))
    dt_local = datetime.datetime(year, month, day, hour, minute)
    # A simple timezone estimation (replace with a more robust solution if needed)
    tz_str = "Asia/Colombo"
    ...
    # 3. Calculate house cusps (for Lagna)
    houses, ascmc = swe.houses(jd_utc, lat, lon, b'P')
    lagna_long = ascmc[0]
    
    # 4. Calculate D1 (Rasi) positions
    d1_planets = {}
    ...
    # 5. Calculate D9 (Navamsa) positions
    d9_planets = {}
...
# --- LLM Helper & Other Endpoints (Unchanged) ---
